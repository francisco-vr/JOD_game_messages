...
function bindata(){
var data = [];
for (i=0;i<nb_bins;i=i+1){data.push(0)}
return data;
}
function bindata_xy(){
var data = [];
...
var initdataspline = [[min, 0],[max, 0]];
    ...
    chart: {
        ...
        events: {
						...
						load:function(e){
            				...
            				this.series[0].update({dragDrop: {dragMaxY:this.yAxis[0].getExtremes().max}});
                        ...
                        var bins = this.series[1];
                        var canvas = this.series[2];
						var sub = bins.yData.map(function (num, idx) {
...
$('#remove_component').on('click', function(){
	...
	slider[0].destroy();
	sliders.pop();
	left_handles.pop();centers.pop();right_handles.pop();weights.pop();
	$('.slider').last().remove();
